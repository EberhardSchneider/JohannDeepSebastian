...
def vectorizePart(part):
    ...
    for measure in measures:
        for note in measure.notesAndRests:
            print(note.pitch.midi, note.offset + measure.offset)
...
sopran = iterator[1].parts[0]
sequence = vectorizePart(sopran)

fig = plt.figure()
ax = fig.add_subplot(111)
ax.imshow(sequence, aspect='auto', cmap=plt.cm.get_cmap('Greys'), interpolation='nearest')
plt.show()        
